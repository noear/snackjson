 ...
 *
 */
public class Issue_IIGHAX_3 {
    @Test
    public void case31() throws Throwable {
        ONode oNode = ONode.ofJson(json);
        String tasksJson = oNode.get("function").get("arguments").get("tasks").getString();

        String tasksJson2 = JsonReader.read(tasksJson, Options.of(Feature.Read_AutoRepair)).toJson();

        System.out.println(tasksJson2);

        System.out.println("tasksJson2.length() > tasksJson.length(): " + tasksJson2.length() + ">" + tasksJson.length());
        assert tasksJson2.length() > tasksJson.length();
    }
